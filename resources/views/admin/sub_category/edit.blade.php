            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if ($errors->any())
                        ...
                        @endforeach
                    @endif
                    <form class="form-horizontal" action="{{ route('admin.sub_category.update', $model->id) }}" method="post" enctype="multipart/form-data">
                        {{ method_field('put') }}
                        {{ csrf_field() }}
                            ...
                            <div class="controls">
                                <select class="form-control" id="category" name="category_article_id" required>
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}" {{ ($category->id == $model->category_article_id) ? 'selected':'' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="controls">
                                <textarea class="form-control" id="desc" name="description" cols="30" rows="10" placeholder="Description of the category" required>{{ $model->description }}</textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-form-label" for="img-container">Photo</label>
                            <div class="controls">
                                <img class="img-fluid" id="img-container" alt="Speaker " width="100" height="100" src="{{ asset($model->showImage()) }}" />
                                <input type="file" onchange="document.getElementById('img-container').src = window.URL.createObjectURL(this.files[0])" name="image">
                            </div>
